...
##  
##  This particular program replicates the empirical work reported in that paper.
##  For a more generic version of the program, use "tar_ci.prg".
##  
###########################################################################
...
coint <- 0		# set to 1 to estimate cointegrating vector
...
if (coint==1){
  mtit <- rbind("Concentrated Negative Log Likelihood","Function of Beta")
...
mtit <- rbind("Concentrated Negative Log Likelihood","Function of Gamma")
xtit <- "Gamma"
...
yhat2 <- yhat2[wi,]
wi <- w[wi]
ylong <- paste(c("R_"),long,c("(t-1)"),sep="")
yshort <- paste(c("R_"),short,c("(t-1)"),sep="")
xtit <- paste(ylong,c("-"),yshort,sep="")  
ylong <- paste(c("D"),ylong,sep="") 
yshort <- paste(c("D"),yshort,sep="") 
x11()
par(mfrow = c(2,1))
mtit <- "Long Rate Regression"
ytit <- ylong
plot(wi,yhat2[,1],type="l",ann=0)
title(main=mtit,ylab=ytit,xlab=xtit)
mtit <- "Short Rate Regression"
ytit <- yshort
plot(wi,yhat2[,2],type="l",ann=0)
title(main=mtit,ylab=ytit,xlab=xtit)
if (graph_rotate==1){
  x11()
  par(mfrow = c(2,1))
  mtit <- "Long Rate Regression" 
  ytit <- ylong
  plot(yhat2[,1],wi,type="l",ann=0)
  title(main=mtit,ylab=xtit,xlab=ytit)
  mtit <- "Short Rate Regression"
  ytit <- yshort
  plot(yhat2[,2],wi,type="l",ann=0)
  title(main=mtit,ylab=xtit,xlab=ytit)
}
}
