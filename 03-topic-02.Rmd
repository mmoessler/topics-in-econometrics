...
sandwich___bwNeweyWest <- function (x, order.by = NULL,
  ...
  if (kernel %in% c("Truncated", "Tukey-Hanning")) {
    stop(paste("Automatic bandwidth selection only available for ", dQuote("Bartlett"), ", ", dQuote("Parzen"), " and ", dQuote("Quadratic Spectral"), " kernel. Use ", sQuote("bwAndrews"), " instead.", sep = ""))
  }
  prewhite <- as.integer(prewhite)
  umat <- if (inherits(x, "matrix")) {
    x
